    ...
    public class DashBoardController : CloudControllerDefault
    {
        ...
        public async Task<ActionResult> Index()
        {
            try
            {
                ...
                return View(new DashBoardViewModel(await service.GetUserWebSharedFolders(user), await service.GetUserWebSharedFiles(user), Constants.GetWebControllerAndActionForDetails(), Constants.GetWebControllerAndActionForDownload(), usedPercent, user.UsedSpace, user.MaxSpace));
            ...
            catch (CloudFunctionStopException ex)
            {
                ...
                return View(new DashBoardViewModel(new List<SharedFolder>(), new List<SharedFile>(), Constants.GetWebControllerAndActionForDetails(), Constants.GetWebControllerAndActionForDownload(), 0.0, 0.0, 0.0));
            }
            catch (Exception)
            {
